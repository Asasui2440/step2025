...
class Page:
    def __init__(self: Page, url: str, contents: str, next: Optional[Page]):
        self.url = url
...
class Cache:
    ...
    def access_page(self: Cache, url: str, contents: str) -> None:
        ...
        page = self.cache
        while page:
            if page.url == url:  # find page in cache
                self.size -= 1
                if prev_page:
                    prev_page.next = page.next
                else:
                    self.cache = page.next
                break
            prev_page = page
            page = page.next

        new_page = Page(url, contents, self.cache)
        self.cache = new_page
        self.size += 1
        if self.size > self.cache_size:
            prev_page = None
            page = self.cache
            while page.next:  # try to find the last page
                prev_page = page
                page = page.next
