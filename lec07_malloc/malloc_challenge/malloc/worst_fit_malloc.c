...
void *my_malloc(size_t size) {
  ...
  my_metadata_t *worst_fit_metadata_prev = NULL;
  ...
  while (metadata) {
    if(metadata->size >= size){
      if(!worst_fit_metadata || metadata->size < worst_fit_metadata->size){
        worst_fit_metadata = metadata;
        worst_fit_metadata_prev = metadata_prev;
      }
    }
  ...
  int bin = get_bin_index(worst_fit_metadata->size);
  my_remove_from_free_list(worst_fit_metadata, worst_fit_metadata_prev,bin);
